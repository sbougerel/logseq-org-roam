...
TODO: change this... slowly with :hash, :modified (we may still
need to index to these buffers!)

- `:indexed' is t if the file is already in `org-roam' cache
- `:hash' is the sha256 hash of the file to ensure consistency
- `:modified' is t if a modified buffer visits the file
...
from `org-roam-logseq--inventory-files'.  Afterward the
